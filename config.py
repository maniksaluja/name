...
FSUB_1 = int(getenv('FSUB_1', '-1002374330304')) #This chat will not create join request
FSUB_2 = int(getenv('FSUB_2', '-1002374330304')) #This chat will not create join request
RFSUB  = [int(i) for i in getenv("RFSUB", "0").split()] #Give space spearated value of the channel which you want that user will create a join request for leave it 0 if you don't want any request type force sub
...
BUY_LINK = getenv("Buy Link") #Link to the acc or channel on which user will be redirected when he clicks on Buy now in voice msg
...
LOG_CHANNEL_ID = int(getenv('LOG_CHANNEL_ID', '-1002374330304'))
LOG_CHANNEL_ID2 = int(getenv('LOG_CHANNEL_ID2', '0')) #Keep it 0 if you don't have second log channel
FEEDBACK_CHANNEL = int(getenv("FEEDBACK_CHANNEL", 0)) # Keep it 0 if you don't want to give feed back channel id
AFTER_FEEDBACK = int(getenv("AFTER_FEEDBACK", 0)) #Channel on which post will be forwarded when the owner clicks on approve
AUTO_SAVE_CHANNEL = int(getenv("AUTO_SAVE_CHANNEL", 0)) #Channel where all the content from fsub1 will be saved
